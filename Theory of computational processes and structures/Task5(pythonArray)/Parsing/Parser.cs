	...
	sealed class Parser {
		...
		IStatement ParseStatement() {
			...
			if (SkipIf("=")) {
                ...
                if (SkipIf("["))
                {
                    IExpression[] newObj = new IExpression[0];
                    do
                    {
                        Array.Resize(ref newObj, newObj.Length + 1);
                        newObj[newObj.Length-1] = ParseExpression();
                    } while (SkipIf(","));
                    Expect("]");
                    Expect(";");
                    return new AssignmentMas(identifier.Name, newObj);
                }
                else
                {
       ...
       /* IExpression ParseArrayIndex()
        {
            if (SkipIf("["))
            {
                ...
                if (SkipIf(":"))
                {
                    var expressionRight = ParseExpression();
                    var numberRight = expressionRight as Number;
                    int valueRight;
                    int.TryParse(numberRight.Lexeme, NumberStyles.None, NumberFormatInfo.InvariantInfo, out valueRight);
                    Expect("]");
                    return new ArrayIndex(identifierVariable.Name, int.MinValue, true, valueRight);
                }
                else
                {
                    var expressionleft = ParseExpression();
                    var numberleft = expressionleft as Number;
                    int valueLeft;
                    int.TryParse(numberleft.Lexeme, NumberStyles.None, NumberFormatInfo.InvariantInfo, out valueLeft);
                    if (SkipIf(":"))
                    {
                        if (SkipIf("]"))
                            return new ArrayIndex(identifierVariable.Name, valueLeft, true, int.MinValue);
                        else
                        {
                            var expressionRight = ParseExpression();
                            var numberRight = expressionRight as Number;
                            ...
                            int.TryParse(numberRight.Lexeme, NumberStyles.None, NumberFormatInfo.InvariantInfo, out valueRight);
                            Expect("]");
                            return new ArrayIndex(identifierVariable.Name, valueLeft, true, valueRight);
                        }
                    }
                    else
                    {
                        Expect("]");
                        return new ArrayIndex(identifierVariable.Name, valueLeft, false, int.MinValue);
                    }
                }
            }
            else return ParseEqualityExpression();
        }*/
			...
			while (true) {
                ...
                else if (SkipIf("["))
                {
                    ...
                    if (SkipIf(":"))
                    {
                        ...
                        var numberRight = expressionRight as Number;
                        int valueRight;
                        int.TryParse(numberRight.Lexeme, NumberStyles.Number, NumberFormatInfo.InvariantInfo, out valueRight);
                        Expect("]");
                        expression = new ArrayIndex(identifierVariable.Name, 0, true, valueRight);
                    }
                    else
                    {
                        var expressionleft = ParseExpression();
                        var numberleft = expressionleft as Number;
                        int valueLeft;
                        int.TryParse(numberleft.Lexeme, NumberStyles.Number, NumberFormatInfo.InvariantInfo, out valueLeft);
                        if (SkipIf(":"))
                        {
                            if (SkipIf("]"))
                                expression = new ArrayIndex(identifierVariable.Name, valueLeft, true, -1);
                            else
                            {
                                var expressionRight = ParseExpression();
                                var numberRight = expressionRight as Number;
                                ...
                                int.TryParse(numberRight.Lexeme, NumberStyles.Number, NumberFormatInfo.InvariantInfo, out valueRight);
                                Expect("]");
                                expression = new ArrayIndex(identifierVariable.Name, valueLeft, true, valueRight);
                            }
                        }
                        else
                        {
                            Expect("]");
                            expression = new ArrayIndex(identifierVariable.Name, valueLeft, false, valueLeft);
                        }
                    }
                }
				else {
