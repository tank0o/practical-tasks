        ...
        IExpression ParsePrimary()
        {
            ...
            while (true)
            {
                ...
                else if (SkipIf("["))
                {
                    bool l,r;
                    var identifierVariable = expression as Identifier;
                    IExpression expressionleft = new Number(0, "0");
                    l = SkipIf(":");
                    if (!l)
                        expressionleft = ParseExpression();
                    IExpression expressionRight = new Number(0, "-1");
                    if (r = SkipIf(":"))
                    {
                        if (!SkipIf("]"))
                            expressionRight = ParseExpression();
                        else
                            return new ArrayIndex(identifierVariable.Name, expressionleft, l|r, expressionRight);
                    }
                    Expect("]");
                    expression = new ArrayIndex(identifierVariable.Name, new Number(0, "0"), l|r, expressionRight);
                }
                else
                {
                    break;
