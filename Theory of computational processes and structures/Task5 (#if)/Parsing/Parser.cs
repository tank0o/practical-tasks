...
namespace Lab5.Parsing
{
    sealed class Parser
    {
        ...
        PreBlock ParsePreBlock()
        {
            ...
            return new PreBlock(statements);
        }
        Tuple<string, PreBlock> ParsPreIfStatment()
        {
            var condition = ParseIdentifier();
            var block = ParsePreBlock();
            return new Tuple<string, PreBlock>(condition, block);
        }
        Tuple<string, PreBlock> ParsPreElseStatment()
        {
            var block = ParsePreBlock();
            return new Tuple<string, PreBlock>(null, block);
        }
        PreIfStatements ParsPreIfStatments()
        {
            List<Tuple<string, PreBlock>> preIfStatments = new List<Tuple<string, PreBlock>>();
            preIfStatments.Add(ParsPreIfStatment());
            while (SkipIf("elseif"))
                preIfStatments.Add(ParsPreIfStatment());
            if (SkipIf("else"))
                preIfStatments.Add(ParsPreElseStatment());
            Expect("endif");
            return new PreIfStatements(preIfStatments);
        }
        IStatement ParseStatement()
        {
            if (SkipIf("#"))
            {
                if (SkipIf("if"))
                {
                    return ParsPreIfStatments();
                }
                if (SkipIf("define"))
                {
                    var expressionM = ParseExpression();
                    var identifier = expressionM as Identifier;
                    return new PreAssignment(identifier.Name, true);
                }
                if (SkipIf("undef"))
                {
                    var expressionM = ParseExpression();
                    var identifier = expressionM as Identifier;
                    return new PreAssignment(identifier.Name, false);
                }
            }
            if (SkipIf("if"))
            {
