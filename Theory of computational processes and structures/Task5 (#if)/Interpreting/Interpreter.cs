...
namespace Lab5.Interpreting
{
    sealed class Interpreter : IStatementVisitor, IExpressionVisitor<object>
    {
        ...
        Dictionary<string, bool> preVariables = new Dictionary<string, bool>();
        ...
        void RunPreBlock(PreBlock block)
        {
            foreach (var statement in block.Statements)
            {
                Run(statement);
        ...
        public void VisitPreIf(PreIfStatements preIfStatements)
        {
            foreach (var ifEx in preIfStatements.preIfStatements)
            {
                if (ifEx.Item1 != null )
                {
                    if (preVariables.ContainsKey(ifEx.Item1) && preVariables[ifEx.Item1])
                    {
                        RunPreBlock(ifEx.Item2);
                        break;
                    }
                    else if (!preVariables.ContainsKey(ifEx.Item1))
                        throw MakeError(preIfStatements, $"Переменная в предпроцессе не найдена");
                }
                else
                {
                    RunPreBlock(ifEx.Item2);
        ...
        public void VisitPreAssignment(PreAssignment preAssignment)
        {
            preVariables[preAssignment.Variable] = preAssignment.Expr;
        }
        ...
        Exception MakeError(PreIfStatements preIfStatements, string message)
        {
            return new Exception(LexerUtils.MakeErrorMessage(source, 0, message));
        }
    }
}
